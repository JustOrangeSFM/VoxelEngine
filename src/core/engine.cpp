...
bool Engine::init()
{
  ...
  std::cout << "Window size: " << win_width << "x" << win_height << std::endl;
...
void Engine::run()
{
  ...
  if (WorldSave::loadLevel(save_dir, level))
  {
    ...
    if (WorldSave::loadPlayer(save_dir, pd))
      world.setLoadedPlayer(pd);
    ...
    if (ImGui::BeginTabBar("##tabs"))
    {
      ...
      if (ImGui::BeginTabItem("Debug"))
      {
        ...
        if (ImGui::CollapsingHeader("Player", ImGuiTreeNodeFlags_DefaultOpen))
        {
          ...
          flag("Ground", player->isOnGround());
          ImGui::SameLine(0, 12);
          flag("Water", player->isUnderwater());
          flag("Sprint", player->isSprinting());
          ImGui::SameLine(0, 12);
      ...
      if (ImGui::BeginTabItem("Settings"))
      {
        ...
        if (ImGui::CollapsingHeader("Atmosphere", ImGuiTreeNodeFlags_DefaultOpen))
        {
          ...
          if (ImGui::Button("Freeze time"))
            g_settings.time_scale = 0.0f;
          ImGui::SameLine();
          if (ImGui::Button("Normal speed"))
            g_settings.time_scale = 72.0f;
          ImGui::SameLine();
          if (ImGui::Button("Fast (10x)"))
            g_settings.time_scale = 720.0f;
        }
        ...
        if (ImGui::CollapsingHeader("Shadows", ImGuiTreeNodeFlags_DefaultOpen))
        {
          ...
          int sizeIdx = g_settings.shadow_map_size == 1024   ? 0
                        : g_settings.shadow_map_size == 4096 ? 2
                                                             : 1;
...
void Engine::keyCallback(GLFWwindow *window, int key, int, int action, int)
{
  ...
  // Hotbar slots 1-9
  case GLFW_KEY_1:
    engine->world.getPlayer()->setSelectedHotbarSlot(0);
    break;
  case GLFW_KEY_2:
    engine->world.getPlayer()->setSelectedHotbarSlot(1);
    break;
  case GLFW_KEY_3:
    engine->world.getPlayer()->setSelectedHotbarSlot(2);
    break;
  case GLFW_KEY_4:
    engine->world.getPlayer()->setSelectedHotbarSlot(3);
    break;
  case GLFW_KEY_5:
    engine->world.getPlayer()->setSelectedHotbarSlot(4);
    break;
  case GLFW_KEY_6:
    engine->world.getPlayer()->setSelectedHotbarSlot(5);
    break;
  case GLFW_KEY_7:
    engine->world.getPlayer()->setSelectedHotbarSlot(6);
    break;
  case GLFW_KEY_8:
    engine->world.getPlayer()->setSelectedHotbarSlot(7);
    break;
  case GLFW_KEY_9:
    engine->world.getPlayer()->setSelectedHotbarSlot(8);
    break;
...
void Engine::mouseCallback(GLFWwindow *window, double xpos, double ypos)
{
  ...
  if (engine->world.getPlayer()->isInventoryOpen())
    return;
  if (g_settings.show_debug)
    return;
...
void Engine::mouseButtonCallback(GLFWwindow *window, int button, int action, int)
{
  ...
  if (button == GLFW_MOUSE_BUTTON_LEFT)
  {
    ...
    if (ray.hit && ray.block_type != BlockType::BEDROCK && ray.block_type != BlockType::WATER)
    {
  ...
  else if (button == GLFW_MOUSE_BUTTON_RIGHT)
  {
    ...
    if (ray.hit)
    {
      ...
      if (place_pos.y < 0 || place_pos.y >= kChunkHeight)
        return;
